...
import json
from deezer_python.utils import map_artist_album, map_user_track, map_user_artist, map_user_album, map_user_playlist
from deezer_python.errors import GWAPIError
...
class GW:
    ...
    def api_call(self, method, args=None, params=None):
        ...
        p = {'api_version': "1.0",
             'api_token': 'null' if method == 'deezer_python.getUserData' else self._get_token(),
             'input': '3',
             'method': method}
        p.update(params)
        try:
    ...
    def get_user_data(self):
        return self.api_call('deezer_python.getUserData')

    def get_user_profile_page(self, user_id, tab, limit=10):
        return self.api_call('deezer_python.pageProfile', {'user_id': user_id, 'tab': tab, 'nb': limit})

    def get_child_accounts(self):
        return self.api_call('deezer_python.getChildAccounts')

    def get_track(self, sng_id):
        return self.api_call('song.getData', {'sng_id': sng_id})

    def get_track_page(self, sng_id):
        return self.api_call('deezer_python.pageTrack', {'sng_id': sng_id})
    ...
    def get_album_page(self, alb_id):
        return self.api_call('deezer_python.pageAlbum', {
            'alb_id': alb_id,
            'lang': 'en',
            'header': True,
            'tab': 0
    ...
    def get_artist_page(self, art_id):
        return self.api_call('deezer_python.pageArtist', {
            'art_id': art_id,
            'lang': 'en',
            'header': True,
            'tab': 0
    ...
    def get_playlist_page(self, playlist_id):
        return self.api_call('deezer_python.pagePlaylist', {
